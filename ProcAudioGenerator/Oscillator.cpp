...
double Oscillator::MakeSin(double frequency, double deltaTime, double phase_shift)
{
	return std::sin(hertzToAngularVelocity(frequency) * deltaTime + phase_shift);
}

double Oscillator::MakeSquare(double frequency, double deltaTime, double phase_shift)
{
	return MakeSin(frequency, deltaTime, phase_shift) > 0 ? 0.2 : -0.2;
}

double Oscillator::MakeFMSquare(double frequency, double deltaTime, double phase_shift, double LFOHertz, double LFOAmplitude)
{
	return MakeFMSin(frequency, deltaTime, phase_shift, LFOHertz, LFOAmplitude) > 0 ? 0.2 : -0.2;
}

double Oscillator::MakeTriangle(double frequency, double deltaTime, double phase_shift)
{
	return asin(MakeSin(frequency, deltaTime, phase_shift));
}

double Oscillator::MakeFMTriangle(double frequency, double deltaTime, double phase_shift, double LFOHertz, double LFOAmplitude)
{
...
double Oscillator::MakeSaw(double frequency, double deltaTime, double phase_shift)
{
	return (2.0 / PI) * (frequency * PI * fmod(deltaTime, 1.0 / frequency) - (PI / 2.0f));
}

double Oscillator::MakeFMSaw(double frequency, double deltaTime, double phase_shift, double LFOHertz, double LFOAmplitude)
{
...
double Oscillator::GetValue(double frequency, double deltaTime, OscillatorTypes type, double phase_shift, double LFOHertz, double LFOAmplitude)
{
	switch (type)
	{
	case SIN:
